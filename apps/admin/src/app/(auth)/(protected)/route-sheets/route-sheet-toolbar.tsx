...
export function RouteSheetToolbar({ trip }: RouteSheetToolbarProps) {
  ...
  async function handleDownload() {
    ...
    try {
      /**
       * Calls the Fastify backend which uses Puppeteer to render the
       * route sheet HTML and return a PDF buffer.
       *
       * TODO: replace with real API call:
       * const res = await fetch(
       *   `${process.env.NEXT_PUBLIC_API_URL}/trips/${trip.id}/pdf`,
       ...
       * a.href = url
       * a.download = `RouteDesk-${trip.reference}.pdf`
       * a.click()
       * URL.revokeObjectURL(url)
       */
      await new Promise(r => setTimeout(r, 1200)) // simulate PDF generation
  ...
  function handlePrint() {
    // CSS @media print hides the toolbar and prints only #route-sheet-print
    window.print()
  }
